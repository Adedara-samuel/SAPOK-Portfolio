    ...
    async loadAvailability(month, year) {
        ...
        for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
            ...
            const bookedAppointments = appointments.filter(apt =>
                apt.date === dateString && apt.status !== 'cancelled'
            );
            ...
            this.availableSlots[dateString] = availableSlots.length === 0 ? 'fully-booked' : 'available';
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const calendar = new SimpleCalendar({
        onDateSelect: (date) => {
            ...
            if (dateDisplay) dateDisplay.value = displayDate;
